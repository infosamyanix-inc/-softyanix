...
// Dependency Inversion Principle (DIP): Depend on abstraction/interface for link generation
interface LinkService {
  buildLink(text: string): string;
}

class WhatsAppLinkService implements LinkService {
...
const WhatsAppButton: React.FC = () => {
  const handleRedirect = () => {
    const targetUrl = linkService.buildLink(MESSAGE);
  ...
  return (
    ...
    >
      <span className="absolute inset-0 rounded-full bg-green-500 opacity-20 animate-ping-slow" />
      <svg
